...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'audio_file.create': {
    roles: ['podcast-owner', 'admin'],
    key: 'new-audio-upload',
    tenantPath: ['organization', 'user', 'audio_file'],
    userPath: [],
  },
  'audio_file.update': {
    roles: ['podcast-owner', 'admin'],
    key: 'audio-privacy-update',
    tenantPath: ['organization', 'user', 'audio_file'],
    userPath: [],
  },
  'user.create': {
    roles: ['podcast-owner', 'admin'],
    key: 'new-guest-registration',
    tenantPath: ['organization', 'user'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['podcast-owner'];
const customerRoles: string[] = ['guest'];
const tenantRoles: string[] = ['podcast-owner', 'team-member', 'admin'];
