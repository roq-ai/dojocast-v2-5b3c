...
import { getAudioFiles } from 'apiSdk/audio-files';
import { AudioFileInterface } from 'interfaces/audio-file';

import { AudioFileListPage as ShowCaseList } from 'pages/audio-files';

function DashboardPage() {
  const { session } = useSession();

  const audioFileStat = useSWR<PaginatedInterface<AudioFileInterface>>(
    () => `/audio-files`,
    () => getAudioFiles({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Audio Files',
      result: audioFileStat?.data?.totalCount,
      isLoading: audioFileStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`audio-files`} passHref legacyBehavior>
                  <Button
